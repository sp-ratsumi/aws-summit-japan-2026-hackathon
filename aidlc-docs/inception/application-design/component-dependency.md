...
- **同期直接呼び出し**: 主要 CRUD と Reminder/Termination のコアフロー (Service → Component → Repository)
- **イベント駆動 (EventBridge)**: 監査 / 通知 / アカウント削除 / リアクション→終了動線
...
|---|---|---|---|
| `user.onboarded` | S-01 Onboarding | BE-11 Audit | 新規登録の監査 |
| `profile.updated` | S-01 Onboarding (or BE-02 via middleware) | BE-11 Audit | プロファイル変更の監査 |
| `reaction.leave_now_requested` | S-04 Reaction | S-03 Termination | 早期退勤の先行記録 |
| `user.terminated` | S-03 Termination | BE-11 Audit | 退勤記録の監査 |
...
```
FE-07 SchedulerClient — 18:00:00 detect
   └─► FE-05 TerminationOverlay.show('PUNCTUALITY')
         └─ user clicks 退勤 button
              └─► POST /terminations/clock-out → Lambda: s3_termination
                    ├─ verify_jwt (BE-01)
                    ├─ TerminationRepo.put_if_absent (日次冪等)
                    ├─ BE-04.generate_closing_message (Bedrock, fallback)
                    └─ EventBridge emit user.terminated → BE-11 Audit
              ◄── 200 TerminationResponse { recorded: true, message: "..." }
         └─ overlay shows message, cleared
```
