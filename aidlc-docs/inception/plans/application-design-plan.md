...
- **OnboardingModule** — サインアップ / ログイン / 初回ヒアリング / 定時設定
- **ReminderFeedModule** — コンテンツ提示 UI、アプリ内モーダル / トースト
- **NotificationAdapter** — プラットフォーム差分吸収 (Tauri OS 通知 / Web Notifications API)
- **TerminationOverlayModule** — 定時オーバーレイ + ダミー勤怠画面 + 退勤 SE
- **PhotoManagerModule** — 写真アップロード UI / 一覧 / 削除
- **SchedulerClient** — クライアントサイドで定時カウントダウン + 可視性検知
...
- **ProfileComponent** — UserProfile CRUD (ヒアリング + 定時 + 編集)
- **SchedulerComponent** — 提示レート計算 / 次回提示時刻算出 (参照実装含む)
- **ContentSelectionComponent** — Bedrock プロンプト生成 / 応答整形 / フォールバック
- **ContentRepositoryComponent** — ダミーデータ (飲食 / 趣味 / 電車 / 天気) 管理
- **PhotoComponent** — Pre-signed URL 発行 / メタ CRUD / 削除 / IDOR チェック
- **HistoryComponent** — 提示履歴の永続化 / 読取
- **ReactionComponent** — リアクション記録 / 冪等書込 / カテゴリ重み学習フック
- **TerminationComponent** — 退勤演出生成 (AI 一言) / セッション状態遷移
...
### Service Layer
- **ReminderOrchestrationService** — Scheduler → ContentSelection → Content/Photo → History を統合
- **OnboardingOrchestrationService** — Signup → Hearing → Profile 保存までの flow
- **TerminationOrchestrationService** — 定時到達 → オーバーレイ発火 → 退勤記録 → AI メッセージ
