...
| # | Epic | Goal (コンセプト対応) | MVP/Full 比率 |
|---|---|---|---|
| E1 | オンボーディング | ユーザー登録・"ダメな欲望プロファイル" 収集・定時 (ダメモード突入時刻) 設定 | MVP 寄り |
| E2 | ダメな未来のリマインド体験 | 定時 1 時間前からの **堕落ランプ** で "ダメな未来" を視界に差し込む | MVP 中核 |
| E3 | 堕落ゲート (退勤動線) | 定時到達時の **堕落ゲート** (全画面オーバーレイ + ダミー勤怠) でダメモードへ宣言突入 | MVP 中核 |
| E4 | 家族・ペット写真管理 | D1 欲望の具象化素材を安全にアップロード + AI キャプション | MVP 要 |
...
**コンセプト用語**:
- **ダメな欲望プロファイル** = 初回ヒアリング結果 (D1〜D7 の選好情報)
- **堕落ランプ** = 17:00→18:00 の単調非増加レート (仕事モード段階解体)
- **堕落ゲート** = 定時到達時の全画面オーバーレイ (堕落を本人が宣言するゲート)
- **ダメモード突入** = 退勤打刻ボタン押下によるコンセプト体験の完了

---

# E1. オンボーディング

> ユーザーに最初の 10 分で「このアプリは自分を堕としにきている」と感じてもらい、"ダメな欲望プロファイル" を仕込む。
...
> **so I can** 面倒な確認なしに「ダメになる準備」を開始できる。
...
- **Given** 確認メールが届いたユーザー
  ...
  **Then** アプリに自動ログインし、"ダメな欲望プロファイル" ヒアリング画面に遷移する
...
> **so I can** 毎日の "ダメな未来のリマインド体験" に即座に入れる。
...
## US-01-03: 初回ヒアリング (ダメな欲望プロファイル収集) [MVP]

**Story**
> **When** 新規登録直後にオンボーディングに入ったとき、
> **I want to** 10 問程度のヒアリングで自分の "ダメな欲望" (家族・推し・食事・酒・趣味・通勤) を登録したい、
> **so I can** 自分の生活圏に合わせた "ダメな未来" を受け取れる。
...
- **Given** 初回ログインを終えた直後のユーザー
  **When** ヒアリング画面に到達する
  **Then** 10 問以上のヒアリング (定時・地域・D1 家族/ペット有無・D2 推し・D3 食・**D4 酒 (好むジャンルまたは「飲まない」)**・D5 趣味カテゴリ・D6 使用路線・ダメモード強度希望 等) が表示される
- **Given** ヒアリングを一問ずつ回答中
  **When** 中断してアプリを閉じる
  **Then** 回答済みの項目は DynamoDB に保存され、再起動時に途中から再開できる
- **Given** 全問回答完了
  **When** 「保存」を押す
  **Then** "ダメな欲望プロファイル" が DynamoDB に確定保存され、"ダメな未来" 提示画面に進む
- **Given** プロファイルドメイン生成器で無作為に生成した "ダメな欲望プロファイル"
  **When** 保存 → 読み出しを往復
...
- **Given** D4 酒で「飲まない」を選択したユーザー
  **When** 以降 AI プロンプトが構築される
  **Then** 酒カテゴリはプロンプトから除外される (ユーザー意思の尊重) `[PRV]`

**INVEST Notes**: Small=△ (2-3 days、UI の実装含むが AC で分割可能)

---

## US-01-04: 定時設定 (ダメモード突入時刻) [MVP]

**Story**
> **When** プロファイル画面や設定画面にいるとき、
> **I want to** 自分の定時 = ダメモード突入時刻 (デフォルト 18:00) を設定・変更したい、
> **so I can** 会社独自の就業時間に合わせて堕落ゲートの発動タイミングを調整できる。
...
- **Given** 設定画面を開いたユーザー
  ...
  **Then** DynamoDB の `punctualityTime` が更新され、スケジューラは翌日から `19:00` 起点で動く (堕落ランプは 18:00 から)
- **Given** 不正な時刻 (例: `25:99`)
  **When** 送信される
...
**Story**
> **When** 数週間使って "ダメな欲望プロファイル" が実情と合わなくなったとき、
> **I want to** 後からヒアリング回答を編集したい、
> **so I can** 堕落コンテンツの精度を保てる。
...
- **Given** 編集して保存 (例: D4 酒を「ビール」から「ハイボール」に)
...
> 定時 1 時間前からの **堕落ランプ** でダメな欲望 (D1〜D6) を視界に差し込み、仕事モードを段階的に解体するコア体験。

---

## US-02-01: 動的頻度スケジューラの起動 (堕落ランプ) [MVP]

**Story**
> **When** 自分の定時の 1 時間前になったとき、
> **I want to** 定時が近づくほど頻度が上がるリマインドを自動で受けたい、
> **so I can** 仕事モードから徐々にダメモードへギアを堕とせる (= 堕落ランプ)。
...
- **Given** 定時 `18:00` のユーザーが `17:00` を迎える
  ...
  **Then** 堕落ランプセッションが開始される
- **Given** 起動したセッション
  **When** 経過時間 `t` (分) が進む
  **Then** 次の提示までの間隔は `t` に対して単調非増加である (17:00 時点で 10-15 分間隔、17:40 時点で 2-3 分間隔 = 堕落ランプの不変条件) `[PBT-03]` invariant
- **Given** スケジューラの頻度計算ロジック
  **When** 参照実装 (定数テーブル) と比較
  **Then** 全ての経過時間で同一の次回提示時刻を返す `[PBT-05]` oracle
- **Given** 定時 `18:00` に到達
  **When** その瞬間
  **Then** 提示セッションを停止し、堕落ゲート (E3) にハンドオフする
...
**Story**
> **When** ダメな未来コンテンツを選ぶ直前、
> **I want to** 現在の時刻・(許可があれば) 位置情報・天気 (ダミー) を自動で取得したい、
> **so I can** 「今この状況で堕ちるべき」ダメな欲望を受け取れる。
...
## US-02-03: AI によるダメな未来の選定 (Bedrock) [MVP]

**Story**
> **When** 堕落ランプが提示トリガーを発火したとき、
> **I want to** "ダメな欲望プロファイル"・履歴・現在コンテキストを踏まえて AI がベストな堕落カテゴリを選んでほしい、
> **so I can** 毎回新鮮で自分に刺さる "ダメな未来" を受け取れる。
...
- **Given** スケジューラが発火
  ...
  **Then** ダメ欲望カテゴリ (D1〜D6) と候補 contentId が返る
- **Given** Bedrock が 5 秒以内に応答しない
  **When** タイムアウト
  **Then** フォールバック (ルールベース + テンプレート文言) でコンテンツを選ぶ (堕落の背中押しは止めない) `[SEC-15]`
- **Given** 直近 5 件の提示履歴
  **When** 次を選ぶ
  **Then** 同一カテゴリが 3 回連続にならない (ダメ欲望の多様性 invariant) `[PBT-03]`
- **Given** プロファイルで D4 酒=「飲まない」
  **When** AI プロンプトが構築される
  **Then** D4 カテゴリはプロンプトから除外され、D4 は提示されない (ユーザー意思 invariant) `[PBT-03][PRV]`
...
## US-02-04: ダメな未来コンテンツの提示 UI (アプリ内 / OS 通知) [MVP]

**Story**
> **When** アプリからダメな未来コンテンツを渡されたとき、
> **I want to** アプリ使用中なら画面手前に、非アクティブなら OS 通知で表示したい、
> **so I can** 何をしていても "ダメな欲望" が視界に入り、仕事モードが崩れ始める。
...
- **Given** アプリがフォアグラウンド
  **When** コンテンツが到着
  **Then** モーダル/トーストで中央に表示され、タイトル・本文・画像・ダメ欲望カテゴリが見える
...
- **Given** Web 版で通知権限が拒否されている
  **When** コンテンツが到着
  **Then** フォアグラウンドに戻した際にキューから表示される (堕落の誘いは失わない) `[SEC-15]`
- **Given** 通知をクリック
  **When** アプリが前面に戻る
...
## US-02-05: ユーザーリアクション (見た/無視した/もう堕ちる) [MVP]

**Story**
> **When** ダメな未来コンテンツを見た後、
> **I want to** 「もう堕ちる (= もう帰る)」「あと少し」「興味なし」を軽く選びたい、
> **so I can** 次の提示に反映させられる / いますぐ堕落ゲートに移行できる。
...
- **Given** コンテンツ表示中
  **When** 「もう堕ちる」ボタンを押す
  **Then** リアクション (userId, contentId, action=LEAVE_NOW, timestamp) が DynamoDB に保存され、即座に堕落ゲート (E3) に遷移する
- **Given** 「あと少し」
  **When** 押す
  **Then** スケジューラの次回提示を 5 分遅らせる (堕落ランプの一時停止)
- **Given** 「興味なし」
  **When** 押す
  **Then** そのダメ欲望カテゴリの次回重み付けが低下する
...
## US-02-06: ダメカテゴリ D3 近隣飲食店 (飯テロ・ダミー) [MVP]

**Story**
> **When** 退勤が近づいてきたとき、
> **I want to** 近所の居酒屋・ラーメン・カフェを画像付きで見たい、
> **so I can** 「ちょっと寄って堕ちて帰ろう」と思える。

**Persona**: P1, P2, P4

**Traceability**: FR-02-02 (D3)

**AC**
- **Given** コンテキストに位置情報がある
  **When** D3 食事カテゴリが選ばれる
  **Then** ダミーデータから最寄り 3 件のランダム 1 件を返し、AI が堕落煽り文言を生成する (例「あの二郎が今日は空いてる」)
- **Given** ダミーデータ ロード
  **When** アプリ起動
...
## US-02-07: ダメカテゴリ D2/D5 趣味・推し系 (サウナ/ジム/映画/アニメ/推し/ゲーム・ダミー) [MVP]

**Story**
> **When** 自分の趣味・推しカテゴリに合うトリガーが発火したとき、
> **I want to** 事前仕込みのコンテンツをそれっぽく見たい、
> **so I can** デモとして堕落の雰囲気が伝わり、実用感を味わえる。

**Persona**: P1 (サウナ/ジム), P2 (アニメ/推し), P4 (ゲーム/アニメ/推し)

**Traceability**: FR-02-02 (D2, D5)

**AC**
- **Given** "ダメな欲望プロファイル" に D2/D5 カテゴリ登録あり
  **When** 提示ループでそのカテゴリが選ばれる
  **Then** ダミーデータ (最低 5 件/カテゴリ) からランダム 1 件が AI 堕落煽り文言付きで表示される
- **Given** プロファイルに無いカテゴリ
  **When** 提示ループ
  **Then** 選ばれない (カテゴリ invariant) `[PBT-03]`
- **Given** 全カテゴリで AI 生成された文言
  **When** 文字数/トーン検証
  **Then** 20〜80 文字かつユーモラスな堕落煽り (プロンプトで強制) 範囲 (invariant) `[PBT-03]`

**INVEST Notes**: Small=○ (1-2 days)

---

## US-02-08: ダメカテゴリ D6 電車・天気 (ダミー) [MVP]

**Story**
> **When** 天気が雨予報だったり電車遅延があったりしたとき (ダミー演出)、
> **I want to** 「今帰らないと巻き込まれる」という警告を受けたい、
> **so I can** 物理的な "帰らねば" で堕落の背中を押される。

**Persona**: P1, P2, P3

**Traceability**: FR-02-02 (D6)

**AC**
- **Given** 時刻が 17:45
  **When** 天気ダミーデータで「18:15 から雨」がシードされている
  **Then** AI が「あと 30 分で雨。今堕ちれば濡れない」と生成して提示
...
## US-02-09: ダメカテゴリ D4 酒テロ (ダミー) [MVP]

**Story**
> **When** 夕方になり「帰って一杯やりたい」気持ちが芽生えるとき、
> **I want to** 自分の好きな酒ジャンル (ビール / サワー / ハイボール / 日本酒 / ワイン) の銘柄とグラス画像を見たい、
> **so I can** 仕事モードを一気に剥がして「キンキンの一杯」にダイブできる。

**Persona**: P1 (ビール), P2 (ワイン), P3 (ハイボール), P4 (サワー)

**Traceability**: FR-02-02 (D4 酒)

**AC**
- **Given** プロファイルの `D4 酒` が `「飲まない」` 以外
  **When** 提示ループで D4 カテゴリが選ばれる
  **Then** ダミーデータ (最低 5 件/酒ジャンル = 合計 25 件以上) からユーザー選好ジャンルのランダム 1 件が AI 堕落煽り文言付きで表示される (例「キンキンのプレモル、18:15 から冷えてる」)
- **Given** プロファイルの `D4 酒` = `「飲まない」`
  **When** 提示ループ
  **Then** D4 カテゴリは選ばれない (ユーザー意思 invariant) `[PBT-03][PRV]`
- **Given** 同一酒銘柄の連続提示
  **When** 履歴チェック
  **Then** 30 分以内の重複提示はなし `[PBT-03]`
- **Given** AI 生成文言
  **When** 文字数検証
  **Then** 20〜80 文字かつ "飲酒を楽しませる" トーン (飲酒強要でも健康害でもない) `[PBT-03]`

**INVEST Notes**: Small=○ (1 day)

---

## US-02-10: 提示履歴の閲覧 [Full]

**Story**
> **When** 夜に振り返りたいとき、
> **I want to** 今日表示されたダメな未来コンテンツを一覧で見たい、
> **so I can** どのコンテンツが自分を堕落させたか振り返れる (翌日の創造性の源を特定できる)。
...
- **Given** ログイン済みユーザー
  **When** 履歴画面を開く
  **Then** 直近 7 日分のコンテンツ履歴が時系列で表示され、ダメ欲望カテゴリ (D1〜D6) でフィルタ可能
...
> 定時になったらユーザー本人に「今日の仕事は終わりだ、明日の自分に任せてダメになる」と宣言させる、本プロダクトの締めくくり。

---

## US-03-01: 定時到達で堕落ゲート (全画面オーバーレイ) 発動 [MVP]

**Story**
> **When** 設定した定時 (例 18:00) ぴったりになったとき、
> **I want to** アニメーションと効果音付きで全画面オーバーレイ (堕落ゲート) が発動してほしい、
> **so I can** 「終業だ、ダメになるぞ」と強く認識できる。
...
- **Given** 定時 `18:00` を設定したユーザー
  ...
  **Then** 堕落ゲート (全画面モーダル) がスライドインし、効果音 (デフォルト ON、設定 OFF 可) が鳴り、「今日の仕事は終わりだ、明日の自分に任せてダメになれ」のコピーが表示される
- **Given** オーバーレイ表示
  **When** ESC キー or 閉じるボタン
  **Then** **閉じられない** (本気で堕落を迫るため)、ただしダミー勤怠画面の退勤ボタンを押すと閉じる
- **Given** アプリ非アクティブ状態
  **When** 定時が来る
  **Then** Tauri はウィンドウを強制的にフォアグラウンドに戻し、Web はタブのタイトル点滅 + ネイティブ通知で呼び込む
- **Given** 複数端末 (Tauri + Web) で同時ログイン
  **When** 定時
  **Then** 両方で同時に堕落ゲートが発動する
- **Given** 効果音は CDN 配信
  **When** 読み込まれる
...
## US-03-02: ダミー勤怠画面と退勤ボタン (堕落宣言ボタン) [MVP]

**Story**
> **When** 堕落ゲートが出ているとき、
> **I want to** 目の前に「退勤」ボタン 1 個のダミー勤怠画面が出てほしい、
> **so I can** 自分の手で押して **ダメになる意思表示** を記録できる。
...
- **Given** 堕落ゲート発動中
  **When** ダミー勤怠画面が中央に出る
  **Then** 「退勤打刻」ボタンが 1 個、大きく表示され、ヘッダに「ダメになるためにここを押せ」文言が添えられる
- **Given** 退勤ボタン
  **When** クリックされる
  **Then** ダメモード突入完了画面 (「今日もおつかれ。さあ、明日の自分に任せよう」等 AI 生成の一言) が表示される
- **Given** 退勤ボタンが**自動クリックされる** (悪意ある自動化)
  **When** サーバー側で検知
  **Then** 最低限のハートビート/フォーカス確認で抑制を試みる (完璧でなくて OK、デモ要件) — 堕落の宣言は本人でなければ意味がないため
- **Given** 退勤後の状態
  **When** 再度オーバーレイが出ようとする
  **Then** その日 (日付変わるまで) は再発火しない (日次冪等 `[PBT-04]`)

**INVEST Notes**: Small=○ (1 day)

---

## US-03-03: 早期堕ち (リアクション "もう堕ちる" 起点) [MVP]

**Story**
> **When** ダメな未来コンテンツを見て「もう堕ちるぞ」と決意したとき、
> **I want to** 定時前でも堕落ゲートを自分から起動したい、
> **so I can** 今の勢いのまま退勤宣言までダイブできる。
...
- **Given** コンテンツ提示中
  **When** 「もう堕ちる」ボタンを押す
  **Then** US-03-01 と同じ堕落ゲート (全画面オーバーレイ) が即座に発動
- **Given** 早期堕落ゲート
  **When** ダミー勤怠で打刻
  **Then** リアクション "LEAVE_NOW" + Termination "EARLY" が履歴として永続化 `[SEC-03]` (監査ログ、改ざん不可ストレージ) `[SEC-14]`

**INVEST Notes**: Small=○ (0.5 day)

---

## US-03-04: ダメモード突入完了メッセージ (AI 生成) [Full]

**Story**
> **When** 退勤ボタンを押した直後、
> **I want to** その日の自分に合ったねぎらいの一言がほしい、
> **so I can** 気分良くダメモードに突入できる。
...
**AC**
- **Given** 退勤ボタンクリック
  **When** Bedrock に「"ダメな欲望プロファイル" + 天気 + 見たコンテンツ + 今日のダメ欲望カテゴリ分布」を渡す
  **Then** 20-50 文字のねぎらい + 堕落肯定文が生成・表示される (例「今日もよく仕事した。ビール飲んで寝よう、明日の自分が考える」)
- **Given** Bedrock がタイムアウト
  **When** 表示寸前
  **Then** テンプレート文言 ("お疲れさま!今日はダメになろう、明日は頑張ろう") にフォールバック `[SEC-15]`
...
> P3 営業マン + P2 事務の心に刺さる、ダメ欲望 D1 の具象化素材を安全に扱う感情フック担当。
...
> **so I can** "ダメな未来 (D1 家族・ペット)" に自分の大切な存在を混ぜられる。
...
## US-04-02: AI キャプション生成 (D1 ダメ欲望の堕落煽り) [MVP]

**Story**
> **When** 写真が "ダメな未来" として選ばれたとき、
> **I want to** ペット/家族の呼び名や現在時刻を踏まえた堕落煽り一言がキャプションで付いてほしい、
> **so I can** 「今日もミケが待ってる、もう堕ちよう」と心を動かされる。

**Persona**: P2, P3

**Traceability**: FR-02-02 (D1 家族・ペット写真), NFR-05 (PBT)

**AC**
- **Given** 写真が提示コンテンツとして選ばれる
  **When** Bedrock にプロファイル (呼び名) + 時刻 + 画像 ID を渡す
  **Then** 20-60 文字のキャプションが生成される (例「ミケが玄関で待ってる。18:30 までに堕ちれば夕食も一緒」)
...
> **so I can** D1 ダメ欲望の文脈に合わせて選別表示できる。
...
> ユーザーが安心して "ダメな欲望プロファイル" と個人情報を預けられる、透明性の Epic。
...
> **so I can** 安心して許可して "ダメな未来" を受け取れる。
...
**Story**
> **When** 退職・退会したいとき、
> **I want to** 自分のデータ ("ダメな欲望プロファイル" 含む) をすべて消したい、
...
- **Given** 削除受理
  **When** バッチジョブが走る
  **Then** Cognito ユーザー + DynamoDB の全レコード (プロファイル + 履歴 + リアクション + Termination) + S3 の該当プレフィックス画像が削除される
- **Given** 削除操作
  **When** 発生
...
**Story**
> **When** 重大な操作 ("ダメな欲望プロファイル" 変更、写真削除、アカウント削除、堕落ゲート発動) があったとき、
...
| US-01-03 ヒアリング (ダメ欲望プロファイル) | P1,P2,P3,P4 | FR-01-02, NFR-08, NFR-05 (PBT-02, PBT-04, PBT-07) |
| US-01-04 定時設定 | P1,P2,P3,P4 | FR-01-03, NFR-05 (PBT-02) |
| US-01-05 プロファイル編集 | P1,P2,P3,P4 | FR-01-02, NFR-03 (SEC-08) |
| US-02-01 スケジューラ (堕落ランプ) | P1,P2,P3,P4 | FR-02-01, NFR-05 (PBT-03, PBT-05, PBT-06) |
| US-02-02 コンテキスト収集 | P1,P2,P3,P4 | FR-02-03, NFR-08, NFR-03 (SEC-03, SEC-15) |
| US-02-03 AI ダメな未来選定 | P1,P2,P3,P4 | FR-02-01, NFR-03 (SEC-03, SEC-05, SEC-15), NFR-05 (PBT-03) |
| US-02-04 ダメな未来提示 UI | P1,P2,P3,P4 | FR-02-04, NFR-03 (SEC-05, SEC-15) |
| US-02-05 リアクション | P1,P2,P3,P4 | FR-03-02, NFR-05 (PBT-04), NFR-03 (SEC-08) |
| US-02-06 D3 飲食店 | P1,P2,P4 | FR-02-02, NFR-05 (PBT-03) |
| US-02-07 D2/D5 趣味・推し | P1,P2,P4 | FR-02-02, NFR-05 (PBT-03) |
| US-02-08 D6 電車・天気 | P1,P2,P3 | FR-02-02, NFR-05 (PBT-03) |
| US-02-09 **D4 酒テロ** | P1,P2,P3,P4 | FR-02-02 (D4), NFR-05 (PBT-03), NFR-08 (PRV) |
| US-02-10 履歴閲覧 | P1,P2,P3,P4 | FR-03-01, NFR-03 (SEC-08) |
| US-03-01 堕落ゲート発動 | P1,P2,P3,P4 | FR-04-01, NFR-03 (SEC-13) |
| US-03-02 ダミー勤怠 (堕落宣言ボタン) | P1,P2,P3,P4 | FR-04-01, NFR-05 (PBT-04) |
| US-03-03 早期堕ち | P1,P2,P3,P4 | FR-04-01, FR-03-02, NFR-03 (SEC-03, SEC-14) |
| US-03-04 ダメモード突入メッセージ | P1,P2,P3,P4 | FR-02-01, FR-04-01, NFR-03 (SEC-15) |
| US-04-01 写真アップロード | P2,P3 | FR-05-01, NFR-03 (SEC-01, SEC-05, SEC-08, SEC-09), NFR-05 (PBT-02) |
| US-04-02 AI キャプション (D1) | P2,P3 | FR-02-02 (D1), NFR-03 (SEC-08), NFR-05 (PBT-03) |
...
| SEC-08 認可 | US-01-02, US-01-05, US-02-05, US-02-10, US-04-01, US-04-02, US-04-03 | Compliant |
...
| PBT-03 不変条件 | US-02-01 (堕落ランプ), US-02-03, US-02-06, US-02-07, US-02-08, US-02-09 (D4 酒), US-04-02 | Compliant |
| PBT-04 冪等性 | US-01-03, US-02-05, US-03-02 (日次) | Compliant |
| PBT-05 オラクル比較 | US-02-01 (堕落ランプ参照実装) | Compliant |
| PBT-06 状態 PBT | US-02-01 (堕落ランプセッション) | Compliant |
| PBT-07 ジェネレータ品質 | US-01-03 (ダメ欲望プロファイル生成器) | Compliant |
...
| **V** Valuable | ◎ | 全 Story にペルソナとユーザー価値 (= ダメな欲望 の充足) が紐付く |
